...
import static com.user.music.transcript.web.config.KafkaConfigProperties.AUDIO_TRANSCRIPTION_COMPLETED_EVENT_TOPIC;
import static com.user.music.transcript.web.config.KafkaConfigProperties.AUDIO_UPLOAD_EVENT_TOPIC;

@Slf4j
@Configuration
public class AudioTranscriptionCompletedEventListener extends KafkaBaseTemplate {

    @Autowired
    private TopicConfig topicConfig;

    @Override
    @KafkaListener(id = "kafkaAudioTranscriptionCompletedEventFeedData",
            topics = AUDIO_TRANSCRIPTION_COMPLETED_EVENT_TOPIC,
    ...
    public void kafkaMessageListener(String message, Acknowledgment ack) {
        ...
        handleMessage(message, topicConfig.getAudioTranscriptCompletedEventTopic(), ack);
    }
}
